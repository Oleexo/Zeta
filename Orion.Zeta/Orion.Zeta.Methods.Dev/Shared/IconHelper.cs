	...
	public class IconHelper {
		...
		private static Icon GetFolderIcon(IconSize size, FolderType folderType) {
			...
			var res = SHGetFileInfo(@"C:\Windows",
				...
				ref shfi,
				(uint) Marshal.SizeOf((object) shfi),
				flags);
		...
		public static Icon GetIcon(string path, IconSize size = IconSize.Large) {
			...
			switch (size) {
				case IconSize.Large:
					res = SHGetFileInfo(path, 0, ref shinfo, (uint) Marshal.SizeOf((object) shinfo), SHGFI_ICON | SHGFI_LARGEICON);
					break;
				case IconSize.Small:
					res = SHGetFileInfo(path, 0, ref shinfo, (uint) Marshal.SizeOf((object) shinfo), SHGFI_ICON | SHGFI_SMALLICON);
					break;
				default:
